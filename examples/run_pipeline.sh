...
Usage:
  ...
  $0 check-step <rna1|rna2|rna3|rna4|rna5|rna5.1|rna6|rna7.0|rna7|rna7.1|gdna1|gdna2|gdna3|gdna4|spl1|spl2|spl3|spl3.5|spl4> [PATIENT]
  $0 watch-step <rna1|rna2|rna3|rna4|rna5|rna5.1|rna6|rna7.0|rna7|rna7.1|gdna1|gdna2|gdna3|gdna4|spl1|spl2|spl3|spl3.5|spl4> [PATIENT] [INTERVAL_SEC]
  $0 sync
  $0 show-config
...
Examples:
  ...
  $0 check 01-CH-L rna
  $0 check splicing
  $0 check Pat101 splicing
  $0 check-step rna5
  $0 check-step gdna4 01-CH-L
  $0 check-step spl4 Pat101
  $0 watch-step rna7
  $0 watch-step gdna3 01-CH-L 10
  $0 watch-step spl3.5 Pat101 10
USAGE
  exit 0
fi
...
run_research_gather_maf_output() {
  ...
  if [ -n "$sample" ]; then
    PIPELINE_DEFAULTS="$PIPELINE_DEFAULTS" make -C "$REPO" run_research_gather_maf_output CONFIG="$CONFIG" SAMPLE="$sample" OUTDIR="$outdir" $force_arg $skip_running_arg
  else
    PIPELINE_DEFAULTS="$PIPELINE_DEFAULTS" make -C "$REPO" run_research_gather_maf_output CONFIG="$CONFIG" OUTDIR="$outdir" $force_arg $skip_running_arg
  fi
}

run_research_gather_fusion_output() {
  local sample="${1:-}"
  local outdir="${2:-}"

  if [ -n "$sample" ]; then
    PIPELINE_DEFAULTS="$PIPELINE_DEFAULTS" make -C "$REPO" run_research_gather_fusion_output CONFIG="$CONFIG" SAMPLE="$sample" OUTDIR="$outdir" $force_arg $skip_running_arg
  else
    PIPELINE_DEFAULTS="$PIPELINE_DEFAULTS" make -C "$REPO" run_research_gather_fusion_output CONFIG="$CONFIG" OUTDIR="$outdir" $force_arg $skip_running_arg
  fi
}

run_research_gather_mosdepth_output() {
  local sample="${1:-}"
  local outdir="${2:-}"

  if [ -n "$sample" ]; then
    PIPELINE_DEFAULTS="$PIPELINE_DEFAULTS" make -C "$REPO" run_research_gather_mosdepth_output CONFIG="$CONFIG" SAMPLE="$sample" OUTDIR="$outdir" $force_arg $skip_running_arg
  else
    PIPELINE_DEFAULTS="$PIPELINE_DEFAULTS" make -C "$REPO" run_research_gather_mosdepth_output CONFIG="$CONFIG" OUTDIR="$outdir" $force_arg $skip_running_arg
  fi
}
...
watch_step_outputs() {
  ...
  local interval="${3:-5}"
  case "$step" in
    rna1|rna2|rna3|rna4|rna5|rna5.1|rna6|rna7.0|rna7|rna7.1|gdna1|gdna2|gdna3|gdna4|spl1|spl2|spl3|spl3.5|spl4) ;;
  ...
  research)
    ...
    case "$task" in
      ...
      gather_maf_output|gather-maf-output)
        ...
        run_research_gather_maf_output "$sample" "$outdir"
        ;;
      gather_fusion_output|gather-fusion-output)
        sample=""
        outdir=""
        if [ $# -gt 0 ] && [[ "${1:-}" != -* ]]; then
          sample="$1"
        ...
        run_research_gather_fusion_output "$sample" "$outdir"
        ;;
      gather_mosdepth_output|gather-mosdepth-output)
        sample=""
        outdir=""
        if [ $# -gt 0 ] && [[ "${1:-}" != -* ]]; then
          sample="$1"
        ...
        run_research_gather_mosdepth_output "$sample" "$outdir"
        ;;
      strand-blacklist)
        sample=""
        outdir=""
