...
while [ $# -gt 0 ]; do
  case "${1:-}" in
    ...
    --input-suffix) input_suffix_override="${2:-}"; shift 2 ;;
...
spl4_input_suffix="${input_suffix_override:-${splicing_spl4_input_suffix:-.spl3.event_annotated.tsv}}"
...
printf '[spl4] GTF: %s\\n' $(printf '%q' "$gtf_path")
printf '[spl4] FASTA: %s\\n' $(printf '%q' "$fasta_path")
printf '[spl4] input suffix: %s\\n' $(printf '%q' "$spl4_input_suffix")
...
cmd=("\$splicing_python" $(printf '%q' "$script_path") --root $(printf '%q' "$spl4_root") --gtf $(printf '%q' "$gtf_path") --fasta $(printf '%q' "$fasta_path") --sample-filter $(printf '%q' "$target") --output-subdir $(printf '%q' "$output_subdir") --input-suffix $(printf '%q' "$spl4_input_suffix"))
if [ -n "\$spl4_outdir" ]; then
