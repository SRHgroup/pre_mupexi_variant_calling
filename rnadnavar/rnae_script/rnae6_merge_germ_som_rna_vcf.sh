...
usage() {
  cat <<EOF
Usage:
  $0 -g GERMLINE.vcf.gz -d DNA_TUMOUR.vcf.gz -r RNA_TUMOUR.vcf.gz -o merged.vcf.gz [options]
...
Notes:
  - Output sample names default to: DNA_NORMAL, DNA_TUMOR, RNA_TUMOR (override with --out-normal/--out-dna/--out-rna)
...
# -------------------------
# 2) DNA somatic -> DNA_TUMOUR + SOMATIC + SOURCE_SET=SOMATIC (strip AS_*)
# -------------------------
...
dna_norm="${tmpdir}/dna.DNA_TUMOUR.vcf.gz"
bcftools reheader -s "${dna_map}" -o "${dna_norm}" "${dna_sub}"
bcftools index -t "${dna_norm}"

dna_clean="${tmpdir}/dna.DNA_TUMOUR.noAS.vcf.gz"
strip_as_info "${dna_norm}" "${dna_clean}"

dna_tag="${tmpdir}/dna.DNA_TUMOUR.tagged.vcf.gz"
...
# -------------------------
# 3) RNA editing -> RNA_TUMOUR + SOURCE_SET=RNA_EDIT + EDIT_SIG derived from flags (strip AS_*), optionally filter to known sites
# -------------------------
