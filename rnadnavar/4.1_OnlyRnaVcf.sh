...
sample_is_requested() {
  ...
  local candidate="$1"
  [ -z "$requested" ] || [ "$candidate" = "$requested" ]
}
...
while IFS= read -r line; do
  ...
  sample_is_requested "$sample_name" || continue
...
SCRIPT
  ...
  qsub -W group_list="${qsub_group:-srhgroup}" -A "${qsub_account:-srhgroup}" -d "$(pwd)" \
    -l nodes=1:ppn=4,mem=8gb,walltime="00:08:00:00" -r y -N "${prefix}.${name}" -o "$repdir" -e "$repdir" "$runscript"
