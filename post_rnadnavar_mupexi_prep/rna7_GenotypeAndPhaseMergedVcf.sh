...
# rna7: genotype union alleles back into BAMs and phase merged VCF.

usage() {
  cat <<'USAGE'
Usage: bash rna7_GenotypeAndPhaseMergedVcf.sh -c CONFIG [-s SAMPLE] [-f]
USAGE
}
...
: "${rna7_genotyped_vcf_extension:?CONFIG must define rna7_genotyped_vcf_extension}"
...
if [ -z "${sample:-}" ]; then
  ...
  echo "Running rna7 only for $sample"
fi
...
while IFS= read -r line; do
  ...
  genotyped_vcf="${outdir_only}/${name}_${rna7_genotyped_vcf_extension}"
  ...
  if ! precheck_vcfgz "$merged_vcf" "${prefix}.${name}" "run rna6 first"; then
