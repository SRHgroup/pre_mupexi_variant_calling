...
grep -v '^#' $samples | while read sample_name fastq1 fastq2; do
	...
	AFvcf=${vcfdir}/${name}_${output_extension_262}
        nonAFvcf=${vcfdir}/${name}_${output_extension_31}

	outvcf=${vcfdir}/${name}_${output_extension_32}
	...
	script="${logdir}/run.${name}.${prefix}.sh"
		...
		printf "python3 ./AddAFinFormat.py -a $AFvcf -n $nonAFvcf -o $outvcf" >> $script
