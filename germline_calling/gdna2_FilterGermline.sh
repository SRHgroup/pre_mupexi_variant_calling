...
# gdna2: filter germline calls by QC thresholds.

usage() {
  cat <<'USAGE'
Usage: bash gdna2_FilterGermline.sh -c CONFIG [-s SAMPLE_OR_PATIENT] [-f]
USAGE
}
...
while IFS= read -r line; do
  ...
  germvcf="${germline_dir}/${name}_${gdna1_vcf_extension}"
  ...
  if [ ! -s "$germvcf" ]; then
    echo "[precheck] ${prefix}.${name}: missing/empty germline VCF from gdna1: $germvcf" >&2
...
SCRIPT
    ...
    cat <<'SCRIPT'
if [ ! -f "$germvcf" ]; then
  echo "ERROR: missing germline VCF from gdna1: $germvcf" >&2
  exit 1
fi
